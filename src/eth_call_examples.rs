...
pub async fn eth_call_get_token_balance(
    ...
    // First, send eth_call to check balance of token
    ...
    let result = provider
        ...
        .await?;
    info!("Real balance before spoof: {:?}", result);

    // Second, spoof token balance and send eth_call
    let slot = keccak256(&abi::encode(&[
        abi::Token::Address(account),
        abi::Token::Uint(U256::from(balance_slot)),
    ]));
    ...
    state
        .account(token)
        .balance(balance)
        .store(slot.into(), H256::from_low_u64_be(balance.as_u64()));
    let result = provider.call_raw(&tx).state(&state).await?;
    info!("Balance after spoof: {:?}", result);
